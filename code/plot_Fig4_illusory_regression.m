...
load('results/stats_illusory_graded.mat')
preds = stats.pnames([1:2 4 3]);
labels = {'Face-like scores','Face-object categorisation','Luminance','Valence'};
...
for n = 1:2%length(preds)
    ...
    text(0,10e2,labels{n},'Color',co(n,:),'FontSize',18)
    ylabel('BF')
    if n==2
        xlabel('Time (ms)')
    else
...
fn = 'figures/Figure4_IllusoryRegression';
